# your github user name or organization name
GH_OWNER_NAME=""

# direct edit flag is when you create new page and write it, tb2 will add the md file into directory.
# then if this is true, you should where's the new file.
# else if this is false, tb2 will make a directory "DRAFT_PAGES" to your computer, and you will open it easily.
TB2_DIRECT_EDIT=false

# default series name, this will be use for new book creation.
# when you create new book, tb2 will create a github project "SERIESNAME/BOOKNAME".
MDBOOK_SERIES_NAME=""

# default mdbook source directory(empty for use "src")
MDBOOK_SRC_DIR=""

# branch name for mdbook deployment by using tb2
TB2_OPERATION_BRANCH="gh-pages"